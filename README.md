...
兩種卡型共用同一 schema，依 `h1` 前綴由驗證器條件強制：**病毒卡**（科群）與**總論／其他卡**（總論、Prion）。

| 欄位 | 病毒卡 | 總論／其他卡 | 說明 |
|---|:--:|:--:|---|
| `h1` | ✓ | ✓ | 所屬群（須為 `meta.groups` 之一） |
| `name` / `en` / `zh` | ✓ | ✓ | 簡名（唯一）／英文全名／中文全名 |
| `stars` | ✓ | ✓ | 考頻 1–3（★–★★★） |
| `principle` | — | ✓ | 原理／方法（總論、Prion 用；取代病毒欄） |
| `genome` | ✓ | — | 基因體與構造（DNA/RNA、單雙股、正負股、套膜、對稱、複製部位） |
| `transmission` | ✓ | — | 傳播途徑／媒介／宿主 |
| `disease` | ✓ | — | 致病機轉與臨床表現 |
| `lab` | ✓ | — | 實驗室診斷（培養／CPE／包涵體、血清學、分子） |
| `treatment` | ✓ | — | 治療與預防（抗病毒藥、疫苗類型） |
| `interpret` | 選填 | 選填 | 結果判讀（血清標記組合、病毒量等） |
| `pitfall` | 選填 | 選填 | 陷阱／易混點 |
| `compare` | 選填 | 選填 | 鑑別比較表（表頭＋≥1 列） |
| `hot` | ✓ | ✓ | 高頻考點（≥1 條） |
| `qa` | ✓ | ✓ | 歷屆國考題引用（≥1 筆） |
